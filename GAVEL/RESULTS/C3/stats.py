...
BASE_PATH = '/Users/juridirocco/development/ghaction/results/code_comments/Fold_'
result = pd.read_csv(f'{BASE_PATH}1_Results.csv', quotechar='"')
print(len(result.index))
# result = pd.DataFrame()

for i in range(1, 6):
    path = f'{BASE_PATH}{i}_Results.csv'
    remp = pd.read_csv(path, quotechar='"')
    print(len(remp.index))
#     result = pd.concat([result, remp])
